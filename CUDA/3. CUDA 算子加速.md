...
## 4.Reduce based Softmax kernel

$$
Softmax(z_i) = \frac{e^{z_i}}{\sum_j e^{z_j}}
$$

为了防止$z_i,z_j$溢出，我们令$M = \max(z)$，然后改进Softmax为
$$
...
考虑原数据的shape为[batch_size, layer_size]

在创建block时，认为`dim3 block_size(32,k)`，这样我们能够通过`threadIdx.x`获得laneid

然后，我们可以将一个grid对应一个batch_id

> 如果考虑向量化读取，这意味着，block线程数*向量化大小应该为layersize
>
> `k * 32 * vec_size = layer_size`
>
> 然后，gridsize设置一个合适的值，即使一个kernel只能完成总数据一部分，我们也可以循环读取处理完。

kernel的具体流程应该如下：

* 寻找最大值

  * 每个线程Vector读写，找出vector中的最大值

  * 规约找出warp最大值

  * 你可以将所有warp最大值存到`__shared__`中，然后使用一个warp规约计算。这样其实解放了这个block的其他warp一段时间使得他们能够应对其他行的计算。

    你需要谨慎的选取vec_size，使得每个线程给出最大值之后，我们需要的数最好只有32个。

    通常对于1024的参数量，vec_size=1,k=32，正好32个warp获得的结果被一个warp再次规约

    如果是不规则的参数量，使用一个很小的数填充到我们处理的形状即可
...
### template warp_reduce

接下来展示的是模板元编程包装的通用warp_reduce
...
// template<typename> class ReductionOp 模板模板参数，代表ReductonOp接受一个参数，这个参数可以手动填入，也可以使用typename T在上一级模板获得
template<template<typename> class ReductionOp, typename T, int warp_width = WarpSize>
__inline__ __device__ T WarpReduce(T val) {
  for (int laneMask = warp_width / 2; laneMask > 0; laneMask /= 2) {
    // shfl获得当前lain_id 异或 lanemask，例如lamemask=16,
    val = ReductionOp<T>()(val, __shfl_xor_sync(0xffffffff, val, laneMask));
  }
  return val;
}
...
这是一个标准的规约写法，xor目前我只知道这一种用法

* 第一次,线程0获得线程16，线程1获得线程17...
* 第二次，线程0获得线程8，线程1获得线程9...
* 若干次直到线程0获得线程1
* ]这个规约写法可以用在warp 级别加法或者warp
* 通常，你可以用shfl_down代替，但是shfl_xor提供了双向的访问(例如线程16此时用同一个可以访问到线程0),有些时候可能需要用到。
...
> 注意，这个做法细看是， `__shfl_xor_sync(0xffffffff, val, laneMask)`传回的值在这个线程中因为调用了functor，被复制了一次（pass by value），当然这是必要的，不然无法比较。
>
> 也就是说，这里shfl的作用主要是快速获得值，其余计算照常。
...
如果x，y，z都是100维度的float32 vector

那么一共传输的字节就是`100*3*4byte`

一般来说，直接用总字节数除以传输时间（这个加法的时间我们忽略不计，因此是总字节数除以总加法时间（load,calculate,write)）

**因此vector add常常用来测量显存带宽**



具体来说，考虑一个内存事务最大为128bit，也就是4个float32(8bit=1byte)，因此使用向量化float4读写会达到理论最大带宽。

在 GPU 上，L2 缓存位于显存（DRAM）和 L1 缓存（每个多处理器（SM）都有自己的 L1 缓存）之间。L2 缓存的作用是减少显存访问的延迟，并提高内存带宽的利用率。当 GPU 需要访问数据时，它首先检查 L1 缓存，如果未命中，则检查 L2 缓存，最后才访问显存。如果数据被缓存到 L2 缓存中，随后的内存访问可能会命中缓存，从而加快访问速度。

在这里，我们首先需要消除L2 Cache的影响，具体做法就是通过使用不同的内存偏移量（`MEMORY_OFFSET`）来访问显存中的不同位置，可以避免连续的内存访问命中 L2 缓存。这样可以确保每次内存访问都需要从显存中读取或写入数据，而不是从 L2 缓存中获取



简单来说，首先，往GPU启动一次这个kernel，我们称它使用的数据为D0，这样这个L2 cache里面就是D0，**注意一次kernel使用的数据应该小于等于你的L2 cache 大小（如果你的kernel使用的数据比L2 cache大，并且你的下一次读取又不够远，那么你读取的数据就有很大可能已经在L2 cache里面了）**

然后，我们正常启动新的kernel，每次kernel的读应该都会cache miss，因此会从显存读L2 cache大小的数据，然后每个SM再从L2cache读数据。这基本相当于直接从显存读数据了。

在以下的代码中，我们计算了处理10次40M数据的总时间。
...
* **Warp Scheduler** 对warp资源的分配
* **Dispatch port**： 发射warp指令，一个时间周期发射一个
  * 在一个周期内，SM的调度器可以调度多个warp，并通过多个dispatch ports发射多个指令。
  * **NVIDIA Volta和Ampere架构**：每个SM也具有**4个warp调度器**，但**每个调度器可以在一个周期内发射两条指令**。因此，一个SM在一个周期内最多可以发射8条指令。
  * warp调度器会选择当前处于准备状态的warp，并将其指令分配给适当的dispatch ports。这样，多个warp的指令可以在同一个周期内并行执行。每个warp调度器通常有一个指令缓冲区，可以提前加载多个指令，以便在一个warp的指令等待内存访问时，调度器可以切换到另一个warp的指令执行。
* **Register File(**T4中有1w+个)，寄存器资源
* **Function Unit**
  * INT32 CUDA CORE
  * FP32 CUDA CORE
  * FP16 CUDA CORE
...
* 数据，指令是否已经在指令Cache和数据Cache
* Register File数量是否够用
* Function Unit是否足够（计算资源是否足够）
* Dispatch port是否占用（当前Warp是否在排队等待发射，排序是依据驱动层面算法）
...
## 计算GPU fp32 cuda core 峰值算力

==所有SM的所有fp32 CUDA core都在工作==

$FP32\ FLOPS\\=SM数量\\×每个SM的CUDA核心数量\\×每个CUDA核心的FP32运算数\\×核心时钟频率×2$

- **SM数量** : 是GPU中流式多处理器的数量。
- **每个SM的CUDA核心数量**: 每个SM的CUDA核心数量 是每个SM中的CUDA核心数量。
- **每个CUDA核心的FP32运算数**: 每个CUDA核心的FP32运算数 通常为1。
- **核心时钟频率核心时钟频率** : 是CUDA核心的工作频率（通常以GHz为单位）。
- **乘以2**: 是因为CUDA核心在每个时钟周期内可以执行两个浮点运算（FMA操作：一个乘法和一个加法）。

要点：

* 启动每个SM最大线程数（以保证SM的计算资源不会有空）
* 喂满FP32 cuda core 指令吞吐（使用乘加FMA(Fused Multiply Add）)
* PTX指令对计算指令进行==cycle级别计时==，以忽略访存（CUDA中，nvcc编译cuda为ptx，再编译为sass汇编，最后为可执行文件cubin）





以下就是通过测试一个SM的峰值算力来获得理论峰值算力的过程。
...
__global__ void FP32FLOPS(int* start, int* stop, float* x, float* y, float* result) {
    ...
    // A1: 实际上多写一些FMA能够帮助我们忽略for中的判断与加法时间
...
其实因为warp level操作的存在，我们第一个想法就是以warp为单位来处理一个单元的目标数据



以Softmax为例，一个block被设置为32*8，这代表block的每一行我们能使用一个warp来处理一些事，在softmax中我们选择用一个warp来进行warplevel reduce，计算softmax所需要的最大值与数值和。
...
## GPU计算资源占有率 occupancy

$$
occupancy = \frac{active\ warps}{total\ warps}
$$

总的来说，active warp越多越好，这样很容易能掩盖指令延迟和访存延迟。



考虑A100，每个SM有65536个寄存器，最多能并发2048个线程，每个thread使用了100个寄存器，每个block启动了256个thread



因此，每个SM就只能运行(65536/256/100) = 2个block，也就是512个线程并行

此时$occupancy=518/2048=0.25$

这个情况下，除非简化kernel，每个线程只要25个寄存器时才能跑满。

其实，现实情况中，需要考虑其他各种的资源约束。
...
CPU提交CUDA kernel到stream，stream在GPU中启动CUDA kernel，这个提交和启动都是有开销的，这也是为什么我们使用算子融合来消除这个开销。

CUDA stream类似堆列，每个CUDAkernel都提交到CUDA stream排队等待GPU顺序启动与执行。

我们可以利用cuda stream做四种并行优化：

* **CPU上的函数与GPU上的CUDA kernel并行执行**

  * 这个方法是天然存在的，因为是异构系统

    ![image-20240523223019255](./assets/image-20240523223019255.png)

  * 在CPU中，一般有**四个方法**强制阻塞CPU进程来等待GPU的kernel执行完成

    * `cudaDeviceSynchronize()`：它会无条件阻塞主机代码，直到所有在此之前提交的Kernel都完成执行。

    * `cudaMemcpy()`，这个是在默认cuda stream中的操作，这个函数本身是同步的，它会阻塞主机代码**直到之前所有的kernel完成，以及数据传输完成**

      额外的,`cudaMemcpyAsync`在指定流中操作，它等待指定流中**所有之前的提交操作完成。**
      ...
      // 将数据从主机传输到设备，使用指定流
      cudaMemcpyAsync(d_data, h_data, dataSize * sizeof(float), cudaMemcpyHostToDevice, stream);
      // 启动内核，使用指定流
      kernelFunction<<<1, dataSize, 0, stream>>>(d_data);
      // 将结果从设备传输回主机，使用指定流
      cudaMemcpyAsync(h_data, d_data, dataSize * sizeof(float), cudaMemcpyDeviceToHost, stream);
      // 同步指定流，确保所有操作完成
    ...
    * `cudaStreamSynchronize`:指定特定流中的所有操作完成
      ...
      cudaStreamSynchronize(stream);  // 等待指定流中的操作完成
      cudaMemcpy(h_data, d_data, size, cudaMemcpyDeviceToHost);
      
      cudaStreamDestroy(stream);
      ```

* **CPU上的函数与CPU到GPU的数据传输并行**

  * 也就是让cudaMemcpy作为异步执行，而非同步执行

* **CPU到GPU的数据传输和GPU上的CUDA kernel并行**

  * 使用类似pipeline完成，把数据分成三份，一份一份提交

    ![image-20240523223050906](./assets/image-20240523223050906.png)

* **GPU内的多个CUDA kernel并行**

  * 使用特定流

    ```c++
    cudakernel<<<grideSize, blockSize, smemeSize，streamID>>>
    ```

    **`smemSize`**：指定每个线程块动态分配的共享内存大小（以字节为单位）。这是一个可选参数，如果不需要动态共享内存，可以设置为0或省略。

    **`streamID`**：指定内核执行的流（Stream），这是一个 `cudaStream_t` 类型的变量。流用于控制不同任务的执行顺序和并行性。

    - `streamID = 0` 表示使用默认流（默认情况下所有操作都在默认流中执行）。
    - `streamID = myStream` 表示使用自定义流 `myStream`，这可以在主机代码中通过 `cudaStreamCreate` 创建。

    > 动态共享内存（dynamic shared memory）是一种在内核启动时指定大小的共享内存。与静态共享内存不同，动态共享内存的大小可以在运行时根据需要进行调整。
    >
    > 动态共享内存通过在内核函数参数列表中使用 `extern __shared__` 关键字来声明。其大小在内核启动时通过第三个参数 `smemSize` 指定。
    >
    > ```c++
    ...
    > 
    > int main(){
    >     int smemSize = dataSize * sizeof(float);
    >     kernelFunction<<<gridSize, blockSize, smemSize>>>(d_data);
    > }
    > ```

  * 注意，默认流是一个很特殊的流，它承担了流之间synchronize的作用

    1. **如果其他Stream上的操作没结束，default stream 就不会开始**：
       - 当设备上有其他流正在运行时，默认流中的操作会等待其他流中的操作完成之后才开始。这确保了不同流之间的操作顺序和同步性。
    2. **在device上的其他Stream要开始之前，default stream必须先完成**：
       - 在设备上启动其他流的操作之前，默认流中的操作必须完成。这意味着默认流具有较高的优先级，其他流在开始执行前需要等待默认流中的所有操作完成。
    3. **总结：对于default stream，同时刻只有这个流在run**：
       - 在任何时刻，默认流中的操作是独占的，不会与其他流同时运行。这种行为确保了默认流中的操作按顺序进行，不会与其他流的操作并行。

    如下图，stream0会在其他非默认流之前已提交的kernel全部执行完之后才开始执行，并且此时不会有其他流并行执行。

    ![974ff6b71f40150acaa66192695e232](./assets/974ff6b71f40150acaa66192695e232.jpg)
...
> 操作系统的分页
>
> 现代操作系统使用分页机制（paging）来管理内存。分页的主要目的是将物理内存（RAM）分割成小的固定大小的块，称为“页”（page），并将虚拟内存（应用程序看到的内存）映射到这些物理页上。
>
> #### 主要概念
>
> 1. **页（Page）**：内存被分割成固定大小的块，通常为4KB。
> 2. **页表（Page Table）**：每个进程都有一个页表，记录虚拟内存地址与物理内存地址的映射关系。
> 3. **页面交换（Paging/Swapping）**：当物理内存（RAM）不足时，操作系统会将一些不常用的页面暂时移到硬盘上（称为交换区或swap space），并在需要时将它们重新加载到内存中。
>
> #### 分页的影响
>
> 分页机制使得操作系统可以高效地管理内存，但在某些情况下会带来额外的开销：
>
> - **页面错误（Page Fault）**：当进程访问的页面不在物理内存中时，会触发页面错误，操作系统需要将页面从硬盘加载回内存，这会引起延迟。
> - **内存访问延迟**：由于分页，内存访问可能会涉及多次内存访问和地址翻译，这会增加延迟。
>
> ### 页锁定内存的优势
>
> 页锁定内存（pinned memory）是一种特殊的内存分配方式，它告诉操作系统不要将这部分内存分页到硬盘上。这意味着：
>
> 1. **避免页面交换**：页锁定内存不会被操作系统交换到硬盘上，避免了页面错误和交换带来的延迟。
> 2. **直接访问**：GPU可以直接访问这部分内存，而不需要经过复杂的分页机制和地址翻译。
>
> ### 数据传输中的具体优势
>
> 在CUDA编程中，主机（Host）和设备（Device）之间的数据传输是通过DMA（Direct Memory Access）完成的。页锁定内存在数据传输过程中具有显著优势：
>
> 1. **更快的传输速度**：由于页锁定内存不会被分页到硬盘，数据传输可以直接在主机内存和设备内存之间进行，避免了分页带来的延迟。
> 2. **稳定的性能**：页锁定内存提供了稳定的传输性能，因为不会受到页面错误和交换的影响。
...
* 同步API
  ...
  * **cudaDeviceSychronize**：同步到GPU上所有kernel结束

  * **cudaStreamSychronize**： 同步到GPU上某个stream结束

  * **cudaEventSychronize**： Host同步到GPU上某个Event结束

  * **cudaStreamWaitEvent**：让一个流等待一个特定时间发生后才开始

  * **cudaMalloc**

  * **cudaMallocHost**

    * cudaMallocHost在Host上申请了内存，并锁定了这个内存页，确保不会分页

      数据传输过程中，可能会因为操作系统的分页机制而导致额外的开销，影响传输速度。 页锁定内存允许更快的数据传输速度，因为操作系统不会将这部分内存分页到磁盘
    ...
    // 启动内核
    kernelFunction<<<dataSize / 256, 256>>>(d_data);
    // 将结果从设备传回主机
    ...
    * **页锁定还允许异步传输数据，可以与计算任务并行执行**

      根据上面的描述，页锁定的内存不会被操作系统分页到磁盘上，意味着**如果GPU直接访问这个部分，不会发生页面错误**

      因此，这部分内存就支持DMA（Direct Memory Access)，意味着数据传输可以不用CPU调度，从而使用cudaMemcpyAsync实现异步的数据传输
...
* 异步API：
  * **cudaMemcpyAsync**(&to, &from, cudaMemcpyHostToDevice, stream_id)：异步的数据传输
...
* 异步操作(cuda kernel & cudaMemcpyAsync) 只能在非默认流执行
* **cudaMemcpyAsync操作的host memory必须为pinned memory**，使用cudaMallocHost或cudaHostAlloc分配

**正确性原因**：malloc分配的host memory为pageable memory，host操作系统可能会动这块pageable memory，造成异步copy到GPU的数据不正确，所以pinned

**性能原因：**

* **Pinned memory由CUDA API在host上分配，host操作系统不能操作它**，该内存始终驻留在physical memory，可以通过DMA直接在host和GPU之间copy速度更快

* 因为CPU不介入，因此传统的流程（CPU读数据加入buffer，GPU从buffer读数据）就不成立了，**省去临时buffer的创建和销毁，以及pageable memory到临时buffer的拷贝**
...
* 实际上启动kernel或者cudaMemcpyAsync都是指定流的，**因此调用这个api相当于向对应的流中压了一个新的任务，在同一个流中他们是同步的。**

* 所有的流并不是完全异步的，他们的操作需要被Copy Engine和Kernel Engine认领，从而执行操作，因此，你认为控制他们认领的次序，尽可能解耦，就是优化的关键，详细看下面的例子

  在某些架构中，实际上Copy Engine会被特化为H2D和D2H两种

* **流是对操作处理顺序的约束，同一个流操作同步，不同流操作异步**

  对于流中的三个处理（data copy to device, active kernel, data copy to host)，他们实际上是同步处理的，这意味着他们严格按照推入顺序等待上一个操作完成后处理。

  其中，active kernel这个操作包括

  * scheduler为kernel分配资源
  * kernel计算
  * 返回kernel计算完毕的标志

  然后，你就可以继续data copy to host了。

  这也就是为什么在默认流中，cudaMemcpy可以作为同步（因为在流中它本来就是一个同步操作），以及kernel为什么只能顺序执行。

  ==如果使用不同流，实际上你的下一个kernel并不需要等待上一个kernel的data copy to host完成，就可以被scheduler分配资源，这就是最大的改进==

![image-20240524001811133](./assets/image-20240524001811133.png)

总结一下，如上图

==每个kernel是一个操作queue，他们将自己的操作在每个时间周期交给copy engine和kernel engine，这两个也是队列queue，这样，我们将任务分解为了更细的粒度，来交给copy engine和kernel engine，使得pipeline更紧凑==
...
知道了这点后我们可以考虑以下的这两个写法

![image-20240523235605236](./assets/image-20240523235605236.png)

* 第一个写法中，我们使用for循环来为每个流分派任务，你可以看成为每个流的任务队列压入了：复制数据，启动kernel，复制数据
* 第二个写法中，我们首先为每个流分派复制数据，然后为每个流分派启动kernel，然后为每个流分派复制数据

以下展示了两种架构下的流水线可视化

展示了只有一个copy engine，和特化了d2h,h2d copy engine的两种架构中，copy engine和kernel engine接收的不同流队列的操作。

![image-20240524002035449](./assets/image-20240524002035449.png)

这个其实十分好理解，在第一个方法中，因为你的流推入顺序是(copy,active ,copy)，因此这代表copy engine中，你第二个流的h2d copy排在第一个流的d2h copy之后



在第二个方法中，则不会有这个问题，因为你的h2d copy是集中推入的，它在d2h copy之前
...
![image-20240524002040514](./assets/image-20240524002040514.png)

在这个框架中，唯一值得注意的是在这种架构下，==NV硬性规定，在批量连续启动kernel时，d2h必须等待所有kernel完成之后才能开始，没有理由==



以上体现了不同架构下不同流写法，**注意kernel实际上不需要等待上一个完成，只要资源足够就能被分配**
...
### CUDA Stream 性能收益和Stream schedule 规则
