...
### 22.5.6 线程池友好的异步方式：`package_task`
...
* **A coroutine handle**
  ...
  * non-owning handle， 用于从外部恢复或销毁co-routine

  * 

  * 协程句柄是对协程实例的引用，允许控制协程的执行，如挂起和恢复。

  * 协程句柄通常由编译器提供，作为协程的入口和管理点。

  * 使用协程句柄可以查询协程状态，如是否完成执行。
...
* **A coroutine state object**
  ...
  * 内部的典型堆分配state(internal typically heap allocated state)

  * 协程状态对象存储协程的所有状态信息，包括

    * copied parameters of a co_routine
    * promise object
    * the representation of the suspension points
    * local variables which lifetime ends before the current suspension point
    * local variables which lifetime exceed the current suspension point

    

  * 当协程挂起时，其状态被保存在状态对象中；当协程恢复时，状态从此对象中恢复。

  * 状态对象的生命周期管理对协程的正确执行至关重要。

  * 示例：

    ```c++
    auto coroutineFunc = []() -> std::coroutine_handle<> {
...
* `assert(handle)`
  ...
  * 它主要用于调试目的，帮助开发者捕捉不应发生的情况。在生产环境中，通常会禁用 `assert`。3
