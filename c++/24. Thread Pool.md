...
> 2. **`std::function` 的要求**：`std::function` 需要其封装的可调用对象是可拷贝的（copy constructible）。换句话说，你可以创建一个可调用对象的副本并存储在 `std::function` 中。
> 3. **`std::packaged_task` 的性质**：然而，`std::packaged_task` 对象是不可拷贝的，只能被移动（move constructible）。这是因为一个任务的所有权和它的完成状态是唯一的，不能简单地被复制。当你尝试将 `std::packaged_task` 对象放入原始的工作队列时，因为它们不满足 `std::function` 需要的可拷贝性质，所以会发生错误。
>
> ### 解决方案：
>
> 要解决这个问题，你需要一个可以存储不可拷贝对象（如 `std::packaged_task`）的工作队列。解决方案涉及以下几个步骤：
>
> 1. **更改工作队列的类型**：你不能再使用 `std::function<void()>` 作为工作队列的条目类型。相反，你需要一个可以存储 `std::packaged_task` 的队列。
>
>    ```
>    cppCopy code
>    thread_safe_queue_space::thread_safe_queue<std::packaged_task<result_type>> work_queue;
>    ```
>
> 2. **提交函数的修改**：在 `submit` 函数中，你创建了一个 `std::packaged_task`，用于异步执行传入的函数 `f`。然后，你尝试将这个 `std::packaged_task` 放入工作队列。
>
>    ```
>    cppCopy codetypedef typename std::invoke_result_t<Function_type, Args...> result_type;
>    std::packaged_task<result_type(Args...)> task(std::bind(f, std::forward<Args>(args)...));
>    std::future<result_type> res = task.get_future();
>    
>    work_queue.push(std::move(task));  // Move the task into the queue
>    ```
>
>    在这里，`std::packaged_task` 被移动到队列中，因为它是不可拷贝的，但可以被移动。这样，任务的所有权和完成状态转移到了队列中的条目。
>
> ### 总结：
>
> 这段话和代码解释了为什么不能将 `std::packaged_task` 放入存储 `std::function<void()>` 的队列，并提供了解决方案：更改队列以直接存储 `std::packaged_task` 对象。这需要调整 `submit` 函数以适应新的队列类型，并确保 `std::packaged_task` 对象被正确地移动到队列中。通过这种方式，你可以异步执行任务并通过返回的 `std::future` 对象检索结果。
