...
1. **`std::vector`**：
   ...
   - **特点**：提供快速随机访问；在末尾插入/删除元素高效；在中间或开始插入/删除元素效率较低。
2. **`std::list`**：
   - **数据结构**：双向链表（Doubly Linked List）。
   - **特点**：在任何位置插入/删除元素都很高效；不支持随机访问。
3. **`std::deque`**（双端队列）：
   - **数据结构**：一种特殊的动态数组，可以在两端高效插入/删除元素。
   - **特点**：结合了 `std::vector` 和 `std::list` 的部分特点，支持快速随机访问。
4. **`std::forward_list`**（C++11）：
   - **数据结构**：单向链表（Singly Linked List）。
   - **特点**：仅支持向前迭代；空间效率更高；只能在头部高效插入/删除元素。
...
1. **`std::set`**：
   - **数据结构**：通常是红黑树（Red-Black Tree）。
   - **特点**：元素自动排序；每个元素只能出现一次。
2. **`std::map`**：
   - **数据结构**：通常是红黑树。
   - **特点**：基于键-值对；键自动排序；每个键只能出现一次。
3. **`std::multiset`** 和 **`std::multimap`**：
   - **数据结构**：通常是红黑树。
   - **特点**：与 `set` 和 `map` 类似，但允许键重复出现。

### 无序关联容器（Unordered Associative Containers）（C++11）

基于哈希表实现，提供平均情况下常数时间的元素访问。

1. **`std::unordered_set`** 和 **`std::unordered_map`**：
   - **数据结构**：哈希表（Hash Table）。
   - **特点**：元素无特定顺序；提供快速的访问和修改操作。
2. **`std::unordered_multiset`** 和 **`std::unordered_multimap`**：
   - **数据结构**：哈希表。
   - **特点**：与 `unordered_set` 和 `unordered_map` 类似，但允许键重复出现。
...
1. **`std::stack`**：
   - **底层数据结构**：可以是 `std::deque`、`std::list` 或 `std::vector`。
   - **特点**：提供后进先出（LIFO）的数据管理。
2. **`std::queue`**：
   - **底层数据结构**：通常是 `std::deque`。
   - **特点**：提供先进先出（FIFO）的数据管理。
3. **`std::priority_queue`**：
   - **底层数据结构**：通常是 `std::vector`，并以堆的形式组织。
   - **特点**：元素按优先级出列。

### 总结

STL 容器提供了丰富的数据结构选择，以满足不同的编程需求。选择合适的容器可以优化程序性能，提高数据处理效率。每种容器都有其特定的用途和性能特点，理解这些特点有助于更高效地利用 STL。
