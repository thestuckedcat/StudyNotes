...
![image-20240226135350910](./assets/image-20240226135350910.png)
...
要求不能有额外空间存储，因此不能使用hashmap记录。

![image-20240224205915329](C:\Users\52985\AppData\Roaming\Typora\typora-user-images\image-20240224205915329.png)

![image-20240224210031388](C:\Users\52985\AppData\Roaming\Typora\typora-user-images\image-20240224210031388.png)

对于环的入口，实际上我们需要根据慢指针与快指针走过的步数来计算



![image-20240224214730308](C:\Users\52985\AppData\Roaming\Typora\typora-user-images\image-20240224214730308.png)

此时,

* 对于快指针与慢指针，其还剩$S2=L-S1$到环入口

我们对$D = nL-S1$变形可得

$D = (n-1)L+(L-S1) = (n-1)L+S2$

因此，此时，如果我们将慢指针往回拉到链表起始点，两边同时以1个节点的速度向前前进，

* 慢指针将在D步到达开始点
* 快指针在D步时到达$S_1 + D = nL$处，也就是环入口

==因此，我们需要做的就是在他们相遇之后，保持快指针位置，并从头开始慢指针，二者同时以1节点的速度前进，将在入口相遇==
...
public:
    ListNode *detectCycle(ListNode *head) {
       ...
       do{
        ...
        } while(fast != nullptr && slow != fast);//必须使用do while，不然会因为slow,fast都初始化为head导致循环不执行
