...
这里的解法没有完全利用完全二叉树的性质。一个方式是使用二分，二分的看某个节点是否存在，并且使用一次根节点到叶子节点来判断。这个过程需要根据需要查询的叶子节点编号直接算出其父母节点，从而以`O(h)`的复杂度访问一次
 ...
 */
class Solution {
    int reverse_dfs(TreeNode* root){
        // 从右子树开始遍历，找到第一个左右子孩子都是空的即可
        // 考虑到完全二叉树左边一定是满的，所以以左边获得最大深度
        int depth = 0;
        ...
        // 判断右边是否也是这个深度，如果是则是完美二叉树，如果不是则开始遍历
        stk = stack<TreeNode*>();
        stk.push(root);
        int depthr = 0;
       ...
       // cout << depth << endl;
        ...
        // 此时最后一层必定有缺，换句话说我们要想知道缺了哪些必须要遍历到那个节点才能知道，因此这就是一个常规遍历
        // 使用层序遍历降低代码复杂性
        ...
        while(!q.empty()){
            int size = q.size();
            // 如果到达最后一层，此处能够这么比较来源于下面先推入left后推入right，永远判断的是最左边的叶子节点
...
* 队列没有新孩子加入，代表是最底层
* 因为是使用队列进行层序遍历，每次必须记录第一个值（最左边的节点），如果满足是最底层那么这个值就是答案
...
其他的题目就是在递归，迭代，前序后序中序层序里面选一个更好写的
...
递归解法，要求每一个cur当作root的子树左右子树高度差不能大于1

如果返回-1代表不平衡，因此可以做适当剪枝(20和22行)
...
迭代解法：后序遍历的时候保存左右子树的深度即可，有点浪费生命不写了
 ...
 */
class Solution {
    void dfs(TreeNode* cur,vector<string>&ans, string s){
        s += to_string(cur->val);
...
public:
    int sumOfLeftLeaves(TreeNode* root) {
        ...
        while(!stk.empty()){
            ...
            if(cur->left){
                stk.push(make_pair(cur->left,true));
            }
...
这一题用递归，主要是题目自己定了很多规则

* 叶子节点才能生效，因此需要额外判断`if(!cur->left && !cur->right)`

* 树为空，即使targetSum=0也是false
...
class Solution {
    bool dfs(TreeNode* cur, int sum, int targetSum){
        ...
        return dfs(cur->left,sum,targetSum) || dfs(cur->right, sum,targetSum);
    }
public:
 ...
 */
class Solution {
    void dfs(TreeNode* cur, vector<int>& path, vector<vector<int>>& ans, int sum, int targetSum){
        ...
        dfs(cur->left,path,ans,sum,targetSum);
        dfs(cur->right,path,ans,sum,targetSum);
...
![image-20240906135526854](./assets/image-20240906135526854.png)

具体想法就是，postorder确定root，然后在Inorder里面确定左右子树大小，进而在PostOrder划分出左右子树，获得左右子树的root。



仔细观察，我们发现，我们只需要Postorder中root的下标，以及包含此root的子树在Inorder的范围，即可递归起来。
...
class Solution {
    ...
    void solve(     TreeNode* cur,
                    ...
                    vector<int>::iterator Inroot,
                    vector<int>::iterator Inleft,
                    vector<int>::iterator Inright,
                    vector<int>::iterator Postroot)
    {
        ...
        int rightTreeSize = Inright - (Inroot+1);
        if(rightTreeSize)
        {
            auto rootright = Postroot - 1;
            auto Inrootright = find(Inroot+1,Inright,*rootright);
            TreeNode *NR =new TreeNode(*Inrootright);
            cur->right = NR;
            solve(  NR,
                    inorder,
                    postorder,
        ...
        if(leftTreeSize){
            auto rootleft = Postroot - rightTreeSize - 1;
            auto Inrootleft = find(Inleft,Inroot,*rootleft);

            TreeNode *NL = new TreeNode(*Inrootleft);
            cur->left = NL;
...
public:
    TreeNode* buildTree(vector<int>& inorder, vector<int>& postorder) {
        ...
        solve(ans,
            inorder,
            postorder,
            find(inorder.begin(),inorder.end(),root),
            inorder.begin(),
            inorder.end(),
            postorder.end()-1);
 ...
 */
class Solution {
    void cstruct(TreeNode* cur, 
                vector<int>& preorder, 
                vector<int>& inorder,
                vector<int>::iterator preRoot,
                vector<int>::iterator Inleft,
                vector<int>::iterator Inright)
    {
        if(Inleft >= Inright){
        ...
        auto Inroot = find(Inleft,Inright,*preRoot);

        int leftTreeSize = Inroot - Inleft;
        int rightTreeSize = Inright - (Inroot+1);

        if(leftTreeSize){
            auto leftRoot = preRoot + 1;
        ...
        if(rightTreeSize){
            auto rightRoot = preRoot + 1 + leftTreeSize;
...
public:
    TreeNode* buildTree(vector<int>& preorder, vector<int>& inorder) {
        TreeNode* ans = new TreeNode(*preorder.begin());

        cstruct(ans,preorder,inorder,preorder.begin(),inorder.begin(),inorder.end());
 ...
 */
class Solution {
    // 给出根节点cur以及左右子树范围，获取左右孩子以及重新划分左右孩子的左右子树范围
    void cstruct(   TreeNode* cur, 
                    vector<int>::iterator leftTreeBegin, vector<int>::iterator leftTreeEnd,
                    vector<int>::iterator rightTreeBegin, vector<int>::iterator rightTreeEnd)
    {
        if(leftTreeBegin < leftTreeEnd)
        {
            auto itLeft = max_element(leftTreeBegin,leftTreeEnd);
            TreeNode* NL = new TreeNode(*itLeft);
            cur->left = NL;

            cstruct(NL,leftTreeBegin,itLeft,itLeft+1,leftTreeEnd);
        }

        if(rightTreeBegin < rightTreeEnd)
        {
            auto itRight = max_element(rightTreeBegin,rightTreeEnd);

            TreeNode* NR = new TreeNode(*itRight);
            cur->right = NR;

            cstruct(NR,rightTreeBegin,itRight,itRight+1,rightTreeEnd);
        }
...
public:
    TreeNode* constructMaximumBinaryTree(vector<int>& nums) {
        ...
        TreeNode* ans = new TreeNode(*it);

        cstruct(ans,nums.begin(),it,it+1,nums.end());
...
基本的思想还是使用一个merge函数递归，merge函数合并当前节点，并调用merge合并左右子树
...
这一题仅需要二叉搜索树的性质即可解答，无论是迭代还是递归都很简单
...
这一题要么递归使用中序遍历看是否是递增序列，要么用递归

用递归需要注意，并非简单的判断左孩子小于根节点，右孩子大于根节点，还需额外维护一个左子树当前最大值，右子树当前最小值
...
class Solution {
    ...
    bool iteration(TreeNode* root){
        ...
        return is_sorted(nums.begin(),nums.end(),[](int a,int b){
            // 需要额外判断a==b为true，此时is_sort认为严格递增
            return a<=b;
        });
    }
...
class Solution {
    int iteration(TreeNode* root){
        ...
        for(int i = 1; i < nums.size();i++){
            if(abs(nums[i]-nums[i-1]) < mmin){
                mmin = abs(nums[i] - nums[i-1]);
            }
        }
        return mmin;
    }
...
这里的`find_ancestor`需要做的是判断cur的左右子树是否存在q与p，如果同时存在那么这个节点就是最近公共祖先



这里需要额外考虑的地方在于如果p是q的祖先



因为我们碰到p或者q或者nullptr就会返回，因此如果p的子树中有q，那么我们只会返回p。



这显然是可接受的，因为p就是最近公共祖先。这建立在**"如果q在p的子树中，那么p就是最近公共祖先，否则在dfs时一定能搜索到q"**这个基本事实
...
我们先写最简单的递归方式，也就是节点能为自身的祖先，保证存在p，q节点，此时上面的论述为基本事实，我们碰到p，q，nullptr返回该值即可。

对于左右子树，返回其中含有q或者p的节点。（这里是用nullptr代表不存在节点，用节点来代表存在节点）
 ...
 */
class Solution {
    TreeNode* find_ancestor(TreeNode* cur, TreeNode* p, TreeNode* q){
        if(cur == p || cur == q || cur == nullptr){
            return cur;
        }
        TreeNode* lft = find_ancestor(cur->left,p,q);
        TreeNode* rht = find_ancestor(cur->right,p,q);
...
对于拓展情况，最直观的，只需要返回一个`pair<TreeNode*,TreeNode*>`即可，这代表这个节点（包含自身）的子树中是否存在p和q（pair是否都为空），前者代表q，后者代表p。

每进入一个节点，搜索左右子树，并比较自己，更新pair的状态。

每个节点对于左右子树提交上来的pair状态不需要再进入这个子树确认，可以分情况讨论

考虑节点N获得了左子树L的pair，标记L子树拥有p和q。N是第一个见到这个状态的节点

* 如果该L子树根节点是p或者q，那么N是其公共祖先
* 如果该L子树根节点不是p或者q，那么违背了N是第一个见到这个状态的节点

我们只需要在第一个见到这个状态的节点记录节点值，然后pair设置为一个标识位一路返回即可
